    ...
    stages{
        ...
        stage('Results') {
            steps {
                ...
                sh 'ssh kong@ip-192-168-7-18.us-west-2.compute.internal "rm -rf /usr/local/share/lua/5.1/kong/plugins/myplugin && cp -R /tmp/kong-plugin-myplugin-0.1.0-1/lua/kong/plugins/myplugin /usr/local/share/lua/5.1/kong/plugins/ && kong reload"'
            }
        }
    }
}
